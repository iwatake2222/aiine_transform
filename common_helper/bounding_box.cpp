...
bool BoundingBoxUtils::CheckRectInScreen(BoundingBox& bbox, int32_t width, int32_t height)
{
    if (bbox.x < 0 || bbox.y < 0) return false;
    if (bbox.x + bbox.w > width) return false;
    if (bbox.y + bbox.h > height) return false;
}